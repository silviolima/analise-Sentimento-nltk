...
class tgApp(object):
    ...
    def analisar_frase(self, widget):
        ...
        print("\n\n")
        print (bcolors.HEADER+"\n\t\tANALISE EFETUADA"+bcolors.ENDC)
        global featureList

        #figsNeg=['glade/imgs/cry.png','glade/imgs/cry2.png']
        #figsPos=['glade/imgs/happy3.png','glade/imgs/happy2.png']
        figsNeg=['glade/imgs/sad.png']
        figsPos=['glade/imgs/happy.png']
        ...
        if ( frase != ""):
            ...
            if (self.opcao == 'dilma' or self.opcao == 'copa' or self.opcao == 'palmeiras' or self.opcao == 'fatec'):
              ...
              #  print("\n\tFeatureListtt: %s "%(features))
                ...
                valor= show_relacao(obter_sentimento_associado(lista_feature_fell,features_msg))
                if(valor == 'true'):
                    ...
                    if ( self.fell == 'negativo'):
                        ...
                        #self.text_area.set_text(frase)
                        #n=randint(0,1)
                        self.image_black.set_from_file(figsNeg[n])
                        self.image_black.show
                        self.image_doubtRight = self.builder.get_object("doubtRight")
                        self.image_doubtRight.set_from_file('glade/imgs/sad.png')
                        self.image_doubtRight.show()
                        self.image_doubtLeft = self.builder.get_object("doubtLeft")
                        self.image_doubtLeft.set_from_file('glade/imgs/sad.png')
                        self.image_doubtLeft.show()
                     
                    else:
                        #n=randint(0,1)
                        self.image_black.set_from_file(figsPos[n])
                        self.image_black.show
                        self.image_doubtRight = self.builder.get_object("doubtRight")
                        self.image_doubtRight.set_from_file('glade/imgs/happy.png')
                        self.image_doubtRight.show()
                        self.image_doubtLeft = self.builder.get_object("doubtLeft")
                        self.image_doubtLeft.set_from_file('glade/imgs/happy.png')
                        self.image_doubtLeft.show()
                else:
                    self.image_doubtRight = self.builder.get_object("doubtRight")
                    self.image_doubtRight.set_from_file('glade/imgs/black.png')
                    self.image_doubtRight.hide()
                    self.image_doubtLeft = self.builder.get_object("doubtLeft")
                    self.image_doubtLeft.set_from_file('glade/imgs/black.png')
                    self.image_doubtLeft.hide()
                    self.image_black.set_from_file('glade/imgs/interrogacao.png')
                    self.image_black.show    
